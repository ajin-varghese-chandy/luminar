...
const STORE_KEY = 'todoboard:simple:v1';
...
let submitSpinner = submitBtn.querySelector('.spinner-border');
...
let delSpinner = confirmDeleteBtn.querySelector('.spinner-border');
...
function rowHTML(todo) {
  ...
  let tagClass = 'badge text-bg-warning';
  let tagText = 'open';
  if (todo.completed) {
    doneClass = ' is-done';
    dotTitle = 'Mark open';
  ...
  let html = '';
  html += '<li class="list-group-item todo-row d-flex align-items-center gap-3' + doneClass + '" data-id="' + todo.id + '">';
  html += '<button class="btn btn-sm btn-outline-secondary border-0 dot" data-action="toggle" type="button" title="' + dotTitle + '">';
  ...
  html += '<span class="me-2">#' + todo.id + '</span>';
  html += '<span class="' + tagClass + '">' + tagText + '</span>';
  html += '</div></div>';
  html += '<div class="d-flex gap-1">';
  html += '<button class="btn btn-sm btn-outline-secondary" data-action="edit" type="button">Edit</button>';
  html += '<button class="btn btn-sm btn-outline-danger" data-action="del" type="button">Delete</button>';
  html += '</div></li>';
  return html;
}
...
async function fetchTodos() {
  ...
  try {
    ...
    todos = merged;
    applyView();
    toast('Fetched ' + todos.length + ' tasks');
  } catch (error) {
